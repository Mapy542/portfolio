...
subdirs = [d for d in CWDContents if os.path.isdir(os.path.join(CWD, d))]
...
for mdFile in mardownFiles:
    try:
        with open(os.path.join(CWD, relativeMarkdown, mdFile), "r", encoding="utf-8") as f:
            content = f.readlines()
            for line in content:
                # regex to find image paths, be file extension aware
                matches = re.findall(
                    r"!\[.*?\]\((.*?\.(?:jpg|jpeg|png|gif|webp|pdf))\)", line, re.IGNORECASE
                )
                if matches:
...
for jsonFile in staticJsonFiles:
    try:
        with open(os.path.join(CWD, relativeMarkdown, jsonFile), "r", encoding="utf-8") as f:
            content = f.read()
            # regex to find image paths in JSON '"image": "path/to/image.jpg"' do not capture the key, just the value
            matches = re.findall(
                r'"image"\s*:\s*"(.*?\.(?:jpg|jpeg|png|gif|webp|pdf))"', content, re.IGNORECASE
            )
            for match in matches:
                imgPath = match.lstrip("/")  # remove leading slash if present
                usedImages.add(imgPath)
...
# find img which are in the images list but not in the usedImages set
extraImages = []
for img in images:
    if img not in usedImages:
        extraImages.append(img)

print("Unneeded images:")
print(extraImages)

# move unneeded images to unneeded folder
unneededFolder = os.path.join(CWD, "unneeded")
if not os.path.exists(unneededFolder):
    os.makedirs(unneededFolder)
for img in extraImages:
    try:
        os.rename(os.path.join(CWD, img), os.path.join(unneededFolder, os.path.basename(img)))
        print(f"Moved {img} to unneeded folder.")
    except FileExistsError:
        os.rename(
            os.path.join(CWD, img),
            os.path.join(unneededFolder, f"duplicate_{os.path.basename(img)}"),
        )
        print(f"Moved {img} to unneeded folder as duplicate_{os.path.basename(img)}.")
